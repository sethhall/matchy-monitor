...
impl FileWatcherHandle {
    pub fn add_file(&self, path: PathBuf, source_name: String, monitor_id: String) -> Result<(), String> {
        self.command_tx.send(WatcherCommand::AddFile { path, source_name, monitor_id })
            .map_err(|e| format!("Failed to send command: {}", e))
    }
    
    pub fn add_directory_pattern(&self, directory: PathBuf, pattern: String, monitor_id: String) -> Result<(), String> {
        self.command_tx.send(WatcherCommand::AddDirectoryPattern { directory, pattern, monitor_id })
            .map_err(|e| format!("Failed to send command: {}", e))
    }
    
    pub fn remove_monitor(&self, monitor_id: String) -> Result<(), String> {
        self.command_tx.send(WatcherCommand::RemoveMonitor { monitor_id })
            .map_err(|e| format!("Failed to send command: {}", e))
    }
    
    pub fn reload_databases(&self) -> Result<(), String> {
        self.command_tx.send(WatcherCommand::ReloadDatabases)
...
fn run_global_file_watcher(
    ...
    let mut worker_opt: Option<(Worker, Extractor)> = if !databases_snapshot.is_empty() {
        match build_worker_and_extractor(&databases_snapshot) {
            Ok(worker_extractor) => {
                eprintln!("[DEBUG] Initialized worker with {} databases", databases_snapshot.len());
    ...
    loop {
        ...
        let timeout = if pending_lines > 0 {
            MAX_BATCH_AGE.checked_sub(last_flush.elapsed()).unwrap_or(std::time::Duration::from_millis(10))
        } else {
        ...
        match command_rx.recv_timeout(timeout) {
            Ok(WatcherCommand::AddFile { path, source_name, monitor_id }) => {
                ...
                if let Ok(mut file) = File::open(&path) {
                    ...
                    watched_files.insert(path.clone(), FileWatchState {
                        path: path.clone(),
                        source_name,
                    ...
                    monitor_files.entry(monitor_id).or_insert_with(Vec::new).push(path.clone());
            ...
            Ok(WatcherCommand::AddDirectoryPattern { directory, pattern, monitor_id }) => {
                eprintln!("[DEBUG] Adding directory pattern: {} / {}", directory.display(), pattern);
                    ...
                    for entry in entries.flatten() {
                        ...
                        if path.is_file() {
                            if let Some(filename) = path.file_name().and_then(|n| n.to_str()) {
                                ...
                                if pattern == "*" || pattern.trim_start_matches("*.").split('.').last() == path.extension().and_then(|e| e.to_str()) {
                                    let source_name = format!("Zeek:{}", filename.trim_end_matches(".log"));
                                    ...
                                    if let Ok(mut file) = File::open(&path) {
                                        ...
                                        monitor_files.entry(monitor_id.clone()).or_insert_with(Vec::new).push(path.clone());
                ...
                eprintln!("[DEBUG] Successfully watching {} files matching pattern {}", matched_count, pattern);
                
                // Watch the directory for new files and track it
                if let Err(e) = watcher.watch(&directory, RecursiveMode::NonRecursive) {
                    eprintln!("[ERROR] Failed to watch directory {}: {}", directory.display(), e);
                } else {
                    watched_directories.entry(monitor_id.clone()).or_insert_with(Vec::new).push(directory.clone());
                }
            }
                ...
                if databases_snapshot.is_empty() {
                    ...
                    match build_worker_and_extractor(&databases_snapshot) {
                        Ok(new_worker_extractor) => {
                            ...
                            eprintln!("[DEBUG] Successfully reloaded {} databases", databases_snapshot.len());
                        }
                        Err(e) => {
            ...
            match event.kind {
                ...
                EventKind::Create(_) => {
                    for event_path in &event.paths {
                        ...
                        if event_path.is_file() && event_path.extension().and_then(|e| e.to_str()) == Some("log") {
                                ...
                                if let Some(parent) = event_path.parent() {
                                    for (mon_id, paths) in &monitor_files {
                                        ...
                                        if paths.iter().any(|p| p.parent() == Some(parent)) {
                                            ...
                                            let filename = event_path.file_name().and_then(|n| n.to_str()).unwrap_or("unknown");
                                            let source_name = format!("Zeek:{}", filename.trim_end_matches(".log"));
                                            
                                            if let Ok(mut file) = File::open(event_path) {
                                                // NEW files: seek to START to capture all data Zeek already wrote
                                                let position = file.seek(SeekFrom::Start(0)).unwrap_or(0);
                                                let inode = metadata(event_path).ok().map(|m| m.ino()).unwrap_or(0);
                                                
                                                eprintln!("[DEBUG] New log file created: {} (inode: {}, starting from position: {})", event_path.display(), inode, position);
                                                watched_files.insert(event_path.clone(), FileWatchState {
                                                ...
                                                monitor_files.entry(mon_id.clone()).or_insert_with(Vec::new).push(event_path.clone());
                                                
                                                if let Err(e) = watcher.watch(event_path, RecursiveMode::NonRecursive) {
                                                    eprintln!("[ERROR] Failed to watch new file {}: {}", event_path.display(), e);
                                                }
                                            }
                                            break;
                                        }
                ...
                EventKind::Modify(_) => {
                    for event_path in &event.paths {
                        if let Some(file_state) = watched_files.get_mut(event_path) {
                            eprintln!("[DEBUG] Processing Modify event for: {}", event_path.display());
                            // Flush previous batch if we're switching files
                            if pending_lines > 0 && (current_monitor_id.as_ref() != Some(&file_state.monitor_id) || current_source.as_ref() != Some(&file_state.source_name)) {
                                if let (Some(src), Some(mon_id)) = (&current_source, &current_monitor_id) {
                                    if let Some((worker, extractor)) = &mut worker_opt {
                                        process_batch_worker(worker, extractor, &buffer, &app, src, window_visible, &monitors, mon_id, None);
                                    }
                                    buffer.clear();
                                    pending_lines = 0;
                            ...
                            if let Ok(m) = metadata(&file_state.path) {
                                ...
                                if new_inode != file_state.inode {
                                    eprintln!("[DEBUG] File rotated: {}", file_state.path.display());
                            ...
                            if let Some(ref mut file) = file_state.file_handle {
                                ...
                                loop {
                                    match file.read(&mut read_buffer) {
                                        Ok(0) => {
                                            eprintln!("[DEBUG] No data available in {}", file_state.path.display());
                                            break;
                                        }
                                        Ok(n) => {
                                            eprintln!("[DEBUG] Read {} bytes from {} (position now: {})", n, file_state.path.display(), file_state.position + n as u64);
                                            ...
                                            if !complete_data.is_empty() || !file_state.incomplete_line.is_empty() {
                                                let mut data_to_process = file_state.incomplete_line.clone();
                                                ...
                                                for line in data_to_process.split(|&b| b == b'\n') {
                                                    ...
                                                    // Update line count
                                                    {
                                                        ...
                                                        if let Some(monitor) = monitors_lock.get_mut(&file_state.monitor_id) {
                                                            let old_count = monitor.stats.lines_processed;
                                                    ...
                                                    if should_flush {
                                                        ...
                                                        if worker_opt.is_none() {
                                                            ...
                                                            if !dbs.is_empty() {
                                                                if let Ok(w) = build_worker_and_extractor(&dbs) {
                                                                    worker_opt = Some(w);
                                                        ...
                                                        if let Some((worker, extractor)) = &mut worker_opt {
                                                            process_batch_worker(worker, extractor, &buffer, &app, &file_state.source_name, window_visible, &monitors, &file_state.monitor_id, None);
                                                        }
                                                        buffer.clear();
                                                        pending_lines = 0;
                                                        last_flush = std::time::Instant::now();
                                ...
                                if path.extension().and_then(|e| e.to_str()) == Some("log") {
                                    if !watched_files.contains_key(&path) {
                                        ...
                                        if let Ok(mut file) = File::open(&path) {
                                            ...
                                            monitor_files.entry(mon_id.clone()).or_insert_with(Vec::new).push(path.clone());
                ...
                if let Some(file_state) = watched_files.get_mut(&path) {
                    ...
                    if let Ok(m) = metadata(&file_state.path) {
                        ...
                        if new_inode != file_state.inode || m.len() < file_state.position {
                            eprintln!("[DEBUG] [SCAN] File rotated or truncated: {}", file_state.path.display());
                            file_state.inode = new_inode;
                            if let Ok(f) = File::open(&file_state.path) {
                                file_state.file_handle = Some(f);
                                file_state.position = 0;
...
fn start_single_monitor(
    ...
    let handle = match monitor.monitor_type {
        ...
        MonitorType::ZeekPacketCapture => {
            if let MonitorConfig::ZeekPacketCapture { interface, zeek_log_dir: _ } = monitor.config {
                std::thread::spawn(move || {
                    // Build worker with current database snapshot
                    let databases_snapshot = databases.lock().unwrap().clone();
                    let (mut worker, extractor) = match build_worker_and_extractor(&databases_snapshot) {
...
fn run_log_file_monitor_worker(
    ...
    if let Err(e) = app_state.file_watcher.add_file(
    ...
    loop {
        let is_enabled = monitors.lock().unwrap()
            .get(&monitor_id)
            .map(|m| m.enabled)
            .unwrap_or(false);
                    ...
                    EventKind::Modify(_) | EventKind::Create(_) => {
                        ...
                        if let Ok(m) = metadata(&file_path) {
                            ...
                            if new_inode != current_inode {
                                eprintln!("[DEBUG] File rotated, reopening: {}", file_path.display());
                            ...
                            match file.read(&mut read_buffer) {
                                ...
                                Ok(n) => {
                                    ...
                                    for line in chunk.split(|&b| b == b'\n') {
                                        ...
                                        if should_flush {
                                            process_batch_worker(worker, extractor, &buffer, &app, &source_name, window_visible, &monitors, &monitor_id, None);
                                            buffer.clear();
                                            pending_lines = 0;
                                            last_flush = std::time::Instant::now();
                                        }
...
fn log_zeek_diagnostics(temp_dir: &std::path::Path, max_lines: usize) {
    ...
    if let Some(tail) = read_last_lines(&stderr_log, max_lines) {
        ...
        eprintln!("[DEBUG] zeek-stderr.log not available at {}", stderr_log.display());
    }
    if let Some(tail) = read_last_lines(&stdout_log, max_lines) {
        eprintln!("[DEBUG] ===== zeek-stdout.log tail (last {} lines) =====\n{}\n==============================================", max_lines, tail);
    } else {
        eprintln!("[DEBUG] zeek-stdout.log not available at {}", stdout_log.display());
    }
}
...
fn run_system_logs_monitor_worker(
    ...
    for line in reader.lines() {
        ...
        if let Ok(line) = line {
            ...
            if should_flush {
                process_batch_worker(worker, extractor, &buffer, &app, "System Logs", window_visible, &monitors, &monitor_id, None);
                buffer.clear();
                pending_lines = 0;
                last_flush = std::time::Instant::now();
            }
...
fn run_zeek_monitor_worker(
    ...
    eprintln!("[DEBUG] Starting Zeek packet capture on interface {}", interface);
    ...
    let (existing_pid, existing_log_dir) = {
        ...
        if let Some(monitor) = monitors_lock.get(&monitor_id) {
            ...
            let log_dir = if let MonitorConfig::ZeekPacketCapture { zeek_log_dir, .. } = &monitor.config {
                zeek_log_dir.clone()
            } else {
                None
            };
    ...
    let (zeek_pid, temp_dir) = if process_running && existing_log_dir.is_some() {
        ...
        let stdout_log = temp_dir.join("zeek-stdout.log");
        let stderr_log = temp_dir.join("zeek-stderr.log");
        eprintln!("[DEBUG] Zeek logs will be written to: {}", temp_dir.display());
    ...
    eprintln!("[DEBUG] Registering Zeek logs with global watcher: {}", temp_dir.display());
    ...
    if let Err(e) = app_state.file_watcher.add_directory_pattern(
        ...
        if let Ok(mut monitors_lock) = monitors.lock() {
            if let Some(monitor) = monitors_lock.get_mut(&monitor_id) {
                monitor.state = MonitorState::Error { 
                    message: format!("Failed to watch Zeek logs via global watcher (will fallback): {}", e)
                };
            }
        }
        ...
        for (log_path, source_name) in [
            ...
            watcher_threads.push(std::thread::spawn(move || {
                ...
                let (mut worker_local, extractor_local) = match build_worker_and_extractor(&databases_clone) {
                    Ok(w) => w,
                    Err(err) => {
                ...
                while !log_path.exists() && attempts < 60 {
                    ...
                    let is_enabled = {
                        ...
                        ml.get(&monitor_id_clone).map(|m| m.enabled).unwrap_or(false)
                    };
    ...
    loop {
        ...
        if !is_enabled {
            ...
            if let Some(pid) = zeek_pid {
                eprintln!("[DEBUG] Requesting elevated privileges to kill Zeek PID {}", pid);
                    ...
                    .output()
                {
                    ...
                    Ok(output) => {
                        eprintln!("[WARN] Failed to kill Zeek: {}", String::from_utf8_lossy(&output.stderr));
                    }
                    Err(e) => {
        ...
        if last_health_check.elapsed() >= HEALTH_CHECK_INTERVAL {
            if let Some(pid) = zeek_pid {
                ...
                if !is_alive {
                    ...
                    eprintln!("[WARN] Preserving Zeek log directory for diagnostics: {}", temp_dir.display());
                ...
                let new_temp_dir = std::env::temp_dir().join(format!("matchy-zeek-{}", uuid::Uuid::new_v4()));
                ...
                match Command::new("osascript").args(["-e", &script]).output() {
                    Ok(output) if output.status.success() => {}
                    Ok(output) => {
                        eprintln!("[ERROR] Failed to start Zeek: {}", String::from_utf8_lossy(&output.stderr));
                        last_health_check = std::time::Instant::now();
                        continue;
                    }
                    Err(e) => {
                ...
                std::thread::sleep(std::time::Duration::from_secs(2));
                if let Some(new_pid) = std::fs::read_to_string(&pid_file).ok().and_then(|s| s.trim().parse::<i32>().ok()) {
                    eprintln!("[INFO] Zeek started with PID {}", new_pid);
                    if let Some(state) = app.try_state::<AppState>() {
                        let mut pids = state.zeek_pids.lock().unwrap();
                        if !pids.contains(&new_pid) { pids.push(new_pid); }
                    }
                    if let Ok(mut monitors_lock) = monitors.lock() {
                        if let Some(monitor) = monitors_lock.get_mut(&monitor_id) {
                    ...
                    eprintln!("[ERROR] Zeek did not write PID; preserving logs at {}", new_temp_dir.display());
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            *databases.lock().unwrap() = loaded_databases;
            ...
            let app_state: tauri::State<AppState> = app.state();
            if database_count > 0 {
                eprintln!("[DEBUG] Notifying global watcher of {} loaded databases", database_count);
        ...
        .run(move |app, event| {
            match event {
                ...
                tauri::RunEvent::Exit | tauri::RunEvent::ExitRequested { .. } => {
                    ...
                    eprintln!("[INFO] To stop packet capture, disable the Zeek monitor in the app.");
                }
                _ => {}
            }
        });
